...
describe('Picture Component', () => {
  ...
  test('renders with correct srcSet for 3x images', () => {
    render(<Picture source1x={source1x} source2x={source2x} source3x={source3x} alt={altText} />);
    const imageElement = screen.getByAltText(altText);
    expect(imageElement).toHaveAttribute('srcset', `${source1x} 1x, ${source2x} 2x, ${source3x} 3x`);
  });

  test('renders with correct srcSet for 2x images', () => {
    render(<Picture source1x={source1x} source2x={source2x} alt={altText} />);
    const imageElement = screen.getByAltText(altText);
    expect(imageElement).toHaveAttribute('srcset', `${source1x} 1x, ${source2x} 2x, ${source2x} 3x`);
  });
